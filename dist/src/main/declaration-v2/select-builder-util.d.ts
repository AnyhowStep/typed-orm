...
export declare namespace SelectBuilderUtil {
    ...
    type WhereIsNotNull<SelectBuilderT extends AnySelectBuilder, TypeNarrowDelegateT extends TypeNarrowDelegate<SelectBuilderT>> = (SelectBuilderT extends SelectBuilder<infer DataT> ? (ReturnType<TypeNarrowDelegateT> extends Column<infer TableAliasT, infer ColumnNameT, infer TypeT> ? SelectBuilder<{
        readonly [key in keyof DataT]: (key extends "joins" ? (JoinCollectionUtil.HasRightJoin<DataT["joins"]> extends true ? JoinCollectionUtil.ReplaceColumnType<DataT["joins"], TableAliasT, ColumnNameT, Exclude<TypeT, null | undefined>> : JoinCollectionUtil.ReplaceColumnType<JoinCollectionUtil.ReplaceNullable<DataT["joins"], TableAliasT, false>, TableAliasT, ColumnNameT, Exclude<TypeT, null | undefined>>) : key extends "selects" ? SelectCollectionUtil.ReplaceSelectType<DataT["selects"], TableAliasT, ColumnNameT, Exclude<TypeT, null | undefined>> : key extends "parentJoins" ? (JoinCollectionUtil.HasRightJoin<DataT["parentJoins"]> extends true ? JoinCollectionUtil.ReplaceColumnType<DataT["parentJoins"], TableAliasT, ColumnNameT, Exclude<TypeT, null | undefined>> : JoinCollectionUtil.ReplaceColumnType<JoinCollectionUtil.ReplaceNullable<DataT["parentJoins"], TableAliasT, false>, TableAliasT, ColumnNameT, Exclude<TypeT, null | undefined>>) : DataT[key]);
