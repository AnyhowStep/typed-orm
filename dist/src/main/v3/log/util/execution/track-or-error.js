...
function trackOrError(log, entityIdentifier, trackRow, connection) {
    return connection.transactionIfNotInOne(async (connection) => {
        const latest = await fetch_latest_or_undefined_1.fetchLatestOrUndefined(log, entityIdentifier, connection);
        if (latest == undefined) {
