...
function calculateWeight(weights, cur, previousTables) {
...
*/
function calculateDeleteOrder(table) {
    const weights = new Map();
    for (let curIndex = 0; curIndex < table.parents.length; ++curIndex) {
        const cur = table.parents[curIndex];
        calculateWeight(weights, cur, table.parents.slice(0, curIndex));
    }
    const arr = [];
    arr.push([table, 0]);
...
exports.calculateDeleteOrder = calculateDeleteOrder;
