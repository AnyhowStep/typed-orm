...
const informationSchema = require("../../../information-schema/execution");
const key_1 = require("../../../key");
async function validateColumns(table, connection, result) {
    const dbColumns = await informationSchema.fetchColumnsOfTable(connection, table.alias);
    ...
    for (let dbColumn of dbColumns) {
        ...
        const codeGenerated = (codeGeneratedOrAutoIncrement &&
            !codeAutoIncrement);
        ...
        const codeExplicitDefault = (codeExplicitDefaultOrGeneratedOrAutoIncrement &&
    ...
    const isColumnOnDb = (columnName) => dbColumns.some(dbColumn => columnName == dbColumn.COLUMN_NAME);
...
async function validateCandidateKeys(table, connection, result) {
    const dbCandidateKeys = await informationSchema.fetchCandidateKeysOfTable(connection, table.alias);
    if (dbCandidateKeys.length > 0) {
        if (table.candidateKeys.length > 0) {
            for (let dbCandidateKey of dbCandidateKeys) {
                if (!key_1.KeyUtil.Array.hasKey(table.candidateKeys, dbCandidateKey.columns)) {
            ...
            for (let codeCandidateKey of table.candidateKeys) {
                if (!key_1.KeyUtil.Array.hasKey(dbCandidateKeys2, codeCandidateKey)) {
