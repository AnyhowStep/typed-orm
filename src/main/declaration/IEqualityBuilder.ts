...
> = (
    (rawIdentifier : IdentifierT) => IExpr<{
        [name in keyof IdentifierT] : (
            MappingT[name] extends IColumn<infer TableNameT, infer NameT, infer TypeT> ?
                {
                    [table in TableNameT] : {
...
> = (
   IEqualityBuilder<
       ConvertFuncT extends undefined ?
           ToEqualityExprFunc<IdentifierT, MappingT> :
           (
               (
                   ConvertFuncT &
                   ToEqualityExprFunc<IdentifierT, MappingT>
               ...
               (
                   ConvertFuncT extends (raw : infer ARawT) => infer AReturnT ?
                       (
                           ToEqualityExprFunc<IdentifierT, MappingT> extends (raw : infer BRawT) => infer BReturnT ?
                               (
